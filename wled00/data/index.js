...
var fxlist = d.getElementById("fxlist"),
  pallist = d.getElementById("pallist");
var cfg = {
  theme: {
    base: "dark",
    bg: { url: "" },
  ...
  if (cfg.comp.hdays) {
    ...
    fetch((loc ? `http://${locip}` : ".") + "/holidays.json", {
      ...
      .catch(function (error) {
        console.log(
          "holidays.json does not contain array of holidays. Defaults loaded."
        );
      })
      .finally(function () {
...
function clearErrorToast() {
  d.getElementById("toast").className = d
...
function checkUsed(i) {
  ...
  if (pJson[id] && (i == 0 || id != i)) {
    d.getElementById(`p${i}warn`).innerHTML = `&#9888; Overwriting ${pName(
      id
    )}!`;
  } else {
...
function populatePresets(fromls) {
  ...
  for (var key of arr || []) {
    ...
    cn += `<div class="segname pname" onclick="setPreset(${i})">${
      isPlaylist(i) ? "<i class='icons btn-icon'>&#xe139;</i>" : ""
    }${pName(i)}</div>
			<i class="icons e-icon flr ${expanded[i + 100] ? "exp" : ""}" id="sege${
      i + 100
    }" onclick="expand(${i + 100})">&#xe395;</i>
...
function populateInfo(i) {
  ...
  cn += `v${i.ver} "${vcn}"<br><br><table class="infot">
	${urows}
	${inforow("Bản build", i.vid)}
	${inforow("Cường độ tín hiệu", i.wifi.signal + "% (" + i.wifi.rssi, " dBm)")}
	${inforow("Thời gian hoạt động", getRuntimeStr(i.uptime))}
	${inforow("Bộ nhớ trống", heap, " kB")}
  	${inforow("Cường độ dòng điện ước lượng", pwru)}
  	${inforow("Khung hình / giây", i.leds.fps)}
	${inforow("Địa chỉ MAC", i.mac)}
	${inforow(
    "Dung lượng bộ nhớ",
    i.fs.u + "/" + i.fs.t + " kB (" + Math.round((i.fs.u * 100) / i.fs.t) + "%)"
  )}
	${inforow("Môi trường", i.arch + " " + i.core + " (" + i.lwip + ")")}
	</table>`;
  ...
  for (var y = 0; y < (s.seg || []).length; y++) {
    ...
    cn += `<div class="seg">
      ...
      inst.bri
				...
				<table class="infot">
					<tr>
						<td class="segtd">LED bắt đầu</td>
						<td class="segtd">${cfg.comp.seglen ? "Length" : "LED Kết thúc"}</td>
						<td class="segtd">Cách ra</td>
					</tr>
					<tr>
						<td class="segtd"><input class="noslide segn" id="seg${i}s" type="number" min="0" max="${
      ledCount - 1
    }" value="${inst.start}" oninput="updateLen(${i})"></td>
						<td class="segtd"><input class="noslide segn" id="seg${i}e" type="number" min="0" max="${
      ledCount - (cfg.comp.seglen ? inst.start : 0)
    }" value="${
      inst.stop - (cfg.comp.seglen ? inst.start : 0)
    }" oninput="updateLen(${i})"></td>
						<td class="segtd"><input class="noslide segn" id="seg${i}of" type="number" value="${
      inst.of
    }" oninput="updateLen(${i})"></td>
					</tr>
				</table>
      ...
      inst.rev ? "checked" : ""
				...
				<label class="check revchkl">
					Mirror effect
					<input type="checkbox" id="seg${i}mi" onchange="setMi(${i})" ${
      inst.mi ? "checked" : ""
    }>
					<span class="checkmark schk"></span>
				</label>
...
function populateEffects(effects) {
  var html = `<div class="searchbar"><input type="text" class="search" placeholder="Tìm kiếm" oninput="search(this)" />
...
function updateTrail(e, slidercol) {
  ...
  e.parentNode.getElementsByClassName("sliderdisplay")[0].style.background =
    val;
}
...
function displayRover(i, s) {
  ...
  d.getElementById(
    "lv"
  ).innerHTML = `WLED đang nhận được dữ liệu ${i.lm} từ ${sour}`;
  d.getElementById("roverstar").style.display =
    i.live && s.lor ? "block" : "none";
}
...
function makeWS() {
  if (ws) return;
  ws = new WebSocket(
    "ws://" + (loc ? locip : window.location.hostname) + "/ws"
  );
...
function readState(s, command = false) {
  ...
  // Palettes
  pallist.querySelector(
    `input[name="palette"][value="${i.pal}"]`
  ).checked = true;
...
function toggleNl() {
  ...
  if (nlA) {
    showToast(
      `Đã bật định giờ. Đèn của bạn sẽ ${nlTar > 0 ? "bật" : "tắt"} ${
        nlMode ? "sau" : "sau"
      } ${nlDur} phút.`
    );
  } else {
    showToast("Bộ định giờ đã tắt.");
  }
  var obj = { nl: { on: nlA } };
  requestJson(obj);
}
...
function toggleSync() {
  ...
  if (syncSend) {
    showToast(
      "Những đèn khác trong cùng mạng sẽ bắt đầu chạy đồng bộ theo đèn này."
    );
  } else {
    showToast(
      "Đèn này và những đèn khác trong cùng mạng sẽ ngừng đồng bộ với nhau."
    );
  }
...
function toggleInfo() {
  ...
  d.getElementById("info").style.transform = isInfo
    ? "translateY(0px)"
    : "translateY(100%)";
...
function toggleNodes() {
  ...
  isNodes = !isNodes;
  d.getElementById("nodes").style.transform = isNodes
    ? "translateY(0px)"
    : "translateY(100%)";
...
function makeSeg() {
  ...
  if (lowestUnused > 0) {
    var pend =
      parseInt(d.getElementById(`seg${lowestUnused - 1}e`).value, 10) +
      (cfg.comp.seglen
        ? parseInt(d.getElementById(`seg${lowestUnused - 1}s`).value, 10)
        : 0);
  ...
  var cn = `<div class="seg">
    ...
    ledCount - 1
						...
						<td class="segtd"><input class="noslide segn" id="seg${lowestUnused}e" type="number" min="0" max="${
    ledCount - (cfg.comp.seglen ? ns : 0)
  }" value="${
    ledCount - (cfg.comp.seglen ? ns : 0)
  }" oninput="updateLen(${lowestUnused})"></td>
					</tr>
				</table>
				<div class="h" id="seg${lowestUnused}len">${ledCount - ns} LED${
    ledCount - ns > 1 ? "s" : ""
  }</div>
...
function resetUtil() {
  var cn = `<button class="btn btn-s btn-i" onclick="makeSeg()"><i class="icons btn-icon">&#xe18a;</i>Thêm phân đoạn</button><br>`;
  d.getElementById("segutil").innerHTML = cn;
}
...
function plR(p) {
  ...
  if (d.getElementById(`pl${p}rptgl`).checked) {
    //infinite
    pl.repeat = 0;
    delete pl.end;
...
function makeP(i, pl) {
  ...
  End preset:<br>
  <select class="btn sel sel-ple" id="pl${i}selEnd" onchange="plR(${i})" data-val=${
  ...
  <div class="po1" id="p${i}o1">
	...
	<div class="c">
		<button class="btn btn-i btn-p" onclick="saveP(${i},${pl})"><i class="icons btn-icon">&#xe390;</i>Save ${
    pl ? "playlist" : i > 0 ? "changes" : "preset"
  }</button>
		${
      i > 0
        ? '<button class="btn btn-i btn-p" id="p' +
          i +
          'del" onclick="delP(' +
          i +
          ')"><i class="icons btn-icon">&#xe037;</i>Delete ' +
          (pl ? "playlist" : "preset")
        : '<button class="btn btn-p" onclick="resetPUtil()">Cancel'
    }</button>
	</div>
...
function makePlEntry(p, i) {
  return `
  <div class="plentry">
    <select class="btn sel sel-pl" onchange="plePs(${p},${i},this)" data-val=${
    plJson[p].ps[i]
  } data-index=${i}>
			${plSelContent}
    </select>
		<button class="btn btn-i btn-xs btn-pl-del" onclick="delPl(${p},${i})"><i class="icons btn-icon">&#xe037;</i></button>
		<div class="h plnl">Duration</div><div class="h plnl">Transition</div><div class="h pli">#${
      i + 1
    }</div><br>
		<input class="noslide pln" type="number" max=6553.0 min=0.2 step=0.1 oninput="pleDur(${p},${i},this)" value=${
    plJson[p].dur[i] / 10.0
  }>
		<input class="noslide pln" type="number" max=65.0 min=0.0 step=0.1 oninput="pleTr(${p},${i},this)" value=${
...
function resetPUtil() {
  var cn = `<button class="btn btn-s btn-i" onclick="makePUtil()"><i class="icons btn-icon">&#xe18a;</i>Tạo bản mẫu</button><br>
            <button class="btn btn-s btn-i" onclick="makePlUtil()"><i class='icons btn-icon'>&#xe139;</i>Tạo danh sách phát</button><br>`;
...
function setSeg(s) {
  ...
  var obj = {
    seg: { id: s, start: start, stop: (cfg.comp.seglen ? start : 0) + stop },
  };
...
function setSegBri(s) {
  var obj = {
    seg: { id: s, bri: parseInt(d.getElementById(`seg${s}bri`).value) },
  };
  requestJson(obj);
}
...
function setPalette(paletteId = null) {
  ...
  d.querySelector(`#pallist .lstI[data-id="${paletteId}"]`).classList.add(
    "selected"
  );
...
function setIntensity() {
  var obj = {
    seg: { ix: parseInt(d.getElementById("sliderIntensity").value) },
  };
  requestJson(obj, false);
}
...
function openGH() {
  window.open(
    "https://nghienrgb.com/t/wled-tong-hop-huong-dan-su-dung-wled-tieng-viet/34"
  );
}
...
function cnfReset() {
  if (!cnfr) {
    ...
    bt.style.color = "#f00";
    bt.innerHTML = "Xác nhận khởi động lại";
    cnfr = true;
    return;
  }
...
function search(searchField) {
  ...
  searchField.parentElement.getElementsByClassName(
    "search-cancel-icon"
  )[0].style.display = searchText.length < 1 ? "none" : "inline";
  var elements =
    searchField.parentElement.parentElement.querySelectorAll(".lstI");
...
function expand(i, a) {
  ...
  d.getElementById("sege" + i).style.transform = expanded[i]
    ? "rotate(180deg)"
    : "rotate(0deg)";
  ...
  if (papi.indexOf("Please") == 0)
    d.getElementById(`p${p}cstgl`).checked = true;
  tglCs(p);
}
...
function size() {
  w = window.innerWidth;
  d.getElementById("buttonNodes").style.display =
    lastinfo.ndc > 0 && w > 770 ? "block" : "none";
...
function togglePcMode(fromB = false) {
  ...
  if (!fromB && ((w < 1250 && lastw < 1250) || (w >= 1250 && lastw >= 1250)))
    return;
  openTab(0, true);
  if (w < 1250) {
    pcMode = false;
  ...
  d.getElementById("bot").style.height =
    pcMode && !cfg.comp.pcmbot ? "0" : "auto";
  sCol("--bh", d.getElementById("bot").clientHeight + "px");
