...
//init ota page
    ...
    server.on(
        "/update", HTTP_POST, [](AsyncWebServerRequest *request)
        {
          if (Update.hasError())
          {
            serveMessage(request, 500, F("Failed updating firmware!"), F("Please check your file and retry!"), 254);
            return;
          }
...
String msgProcessor(const String &var)
{
  if (var == "MSG")
  {
    ...
    {
      messageBody += F("<br><br><form action=/settings><button class=\"bt\" type=submit>Quay lại</button></form>"); //button to settings
    }
    else if (optt == 254)
    {
      messageBody += F("<br><br><button type=\"button\" class=\"bt\" onclick=\"B()\">Quay lại</button>");
    }
    return messageBody;
  }
  return String();
...
void serveSettings(AsyncWebServerRequest *request, bool post)
{
  ...
  { //settings/set POST request, saving
    ...
    case 6:
      ...
      strcpy_P(s2, PSTR("Đang khởi động lại, vui lòng chờ khoảng 10s..."));
      break;
    case 7:
      strcpy_P(s, PSTR("DMX"));
      break;
    ...
    strcat_P(s, PSTR(" cài đặt đã được lưu."));
    if (!s2[0])
      strcpy_P(s2, PSTR("Đang chuyển trang..."));
  ...
  case 7:
    ...
    break;
  case 8:
    request->send_P(200, "text/html", PAGE_settings_um, settingsProcessor);
    break;
  case 255:
